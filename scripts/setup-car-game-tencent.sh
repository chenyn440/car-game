...
# 1) 安装/校验 Nginx
# 2) 安装 car-game systemd 服务
# 3) 写入并启用 Nginx 反代配置
#
# 用法：
# sudo bash scripts/setup-car-game-tencent.sh \
...
if [[ ! -f "$PROJECT_DIR/scripts/car-game.service.template" ]]; then
  echo "缺少模板文件: $PROJECT_DIR/scripts/car-game.service.template" >&2
  exit 1
fi
if [[ ! -f "$PROJECT_DIR/scripts/nginx-car-game.conf.template" ]]; then
  echo "缺少模板文件: $PROJECT_DIR/scripts/nginx-car-game.conf.template" >&2
  exit 1
fi
...
sed \
  ...
  "$PROJECT_DIR/scripts/car-game.service.template" > "$SERVICE_FILE"
...
sed \
  ...
  -e "s|__PROJECT_DIR__|$PROJECT_DIR|g" \
  "$PROJECT_DIR/scripts/nginx-car-game.conf.template" > "$NGINX_CONF"
