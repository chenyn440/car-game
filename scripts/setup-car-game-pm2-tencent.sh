...
if [[ ! -f "$PROJECT_DIR/scripts/car-game.pm2.config.template.cjs" ]]; then
  echo "缺少模板文件: $PROJECT_DIR/scripts/car-game.pm2.config.template.cjs" >&2
  exit 1
fi
...
PM2_CONFIG_FILE="$PROJECT_DIR/scripts/car-game.pm2.config.cjs"
sed \
  -e "s|__APP_NAME__|$APP_NAME|g" \
  -e "s|__PROJECT_DIR__|$PROJECT_DIR|g" \
  -e "s|__SIGNAL_PORT__|$SIGNAL_PORT|g" \
  "$PROJECT_DIR/scripts/car-game.pm2.config.template.cjs" > "$PM2_CONFIG_FILE"
