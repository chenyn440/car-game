...
REMOTE_PROJECT_DIR="/home/car-game"     # e.g. /home/ubuntu/car-game
...
PM2_APP_NAME="leaderboard-api"
API_ENTRY_REL="server/leaderboard-server.mjs"
API_HOST="127.0.0.1"
API_PORT="8787"
RESTART_CMD="source /root/.nvm/nvm.sh >/dev/null 2>&1 || true; nvm use --silent 22 >/dev/null 2>&1 || true; if command -v pm2 >/dev/null 2>&1; then pm2 restart ${PM2_APP_NAME} --update-env || LEADERBOARD_HOST=${API_HOST} LEADERBOARD_PORT=${API_PORT} pm2 start ${REMOTE_PROJECT_DIR}/${API_ENTRY_REL} --name ${PM2_APP_NAME}; elif command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files | grep -q '^${PM2_APP_NAME}\\.service'; then systemctl restart ${PM2_APP_NAME}; else echo 'No pm2 or systemd service found for restart.'; fi"            # optional
